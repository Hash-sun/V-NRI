    ...
    with open(filename) as f:
        ...
        for line in lines:
            ...
            if(line.startswith("MODEL")):
                ...
                if (modelNum % interval == (window_choose+1)):
                    nflag = True
            elif(line.startswith("ATOM") and flag):
                numStep = int(modelNum/interval)
                feature[numStep, 0, int(words[1])-1] = float(words[6])
                feature[numStep, 1, int(words[1])-1] = float(words[7])
                feature[numStep, 2, int(words[1])-1] = float(words[8])
            elif(line.startswith("ATOM") and nflag):
                numStep = int(modelNum/interval)
                feature[numStep, 3, int(words[1])-1] = float(words[6])-feature[numStep, 0, int(words[1])-1]
                feature[numStep, 4, int(words[1])-1] = float(words[7])-feature[numStep, 1, int(words[1])-1]
                feature[numStep, 5, int(words[1])-1] = float(words[8])-feature[numStep, 2, int(words[1])-1]
            elif(line.startswith("TER") and flag):
                flag = False
            elif(line.startswith("TER") and nflag):
                nflag = False
...
features, edges = convert_dataset_md_single(MDfolder, startIndex=1, experiment_size=1, timestep_size=50,
                                            feature_size=feature_size, num_residues=num_residues, interval=train_interval, pdb_start=pdb_start, pdb_end=pdb_end, aa_start=94, aa_end=93+num_residues)
...
features_valid, edges_valid = convert_dataset_md_single(MDfolder, startIndex=1, experiment_size=1, timestep_size=50,
                                                        feature_size=feature_size, num_residues=num_residues, interval=validate_interval, pdb_start=pdb_start, pdb_end=pdb_end, aa_start=94, aa_end=93+num_residues)
...
features_test, edges_test = convert_dataset_md_single(MDfolder, startIndex=1, experiment_size=1, timestep_size=50,
                                                      feature_size=feature_size, num_residues=num_residues, interval=test_interval, pdb_start=pdb_start, pdb_end=pdb_end, aa_start=94, aa_end=93+num_residues)
